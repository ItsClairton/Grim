...
public class PacketEntitySelf extends PacketEntity {
    ...
    protected void initAttributes(GrimPlayer player) {
        ...
        final var scaleAttr = getAttribute(Attributes.GENERIC_SCALE);
        if (scaleAttr == null) {
            throw new IllegalStateException("not found generic scale attribute");
        }

        scaleAttr.withSetRewriter((oldValue, newValue) -> {
